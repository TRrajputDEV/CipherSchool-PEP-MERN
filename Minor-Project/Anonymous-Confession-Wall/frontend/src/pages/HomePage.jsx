...
// The sleek, database-style search bar
    ...
    <div className="max-w-2xl mx-auto mb-6 anim-fade-in relative z-10">
      <form onSubmit={handleSubmit} className="relative group">
        ...
        <div className="relative flex items-center bg-[#0a0a0a] border border-white/10 p-2 sm:p-3 shadow-[0_8px_30px_rgb(0,0,0,0.6)]">
          <span className="text-[#555] pl-3 pr-2 font-mono text-xs tracking-widest">
            {">"}
          </span>
          <input
            type="text"
          ...
          <div className="flex items-center gap-2 pr-2">
            {input && (
              <button
                type="button"
                onClick={handleClear}
                className="text-[#555] hover:text-[#ff4444] px-2 transition-colors font-mono text-[10px] tracking-widest uppercase"
              >
                Clear
              </button>
            )}
            <button
              ...
              className="bg-white/10 hover:bg-white text-white hover:text-black transition-colors px-4 py-2 text-[10px] font-bold tracking-widest uppercase disabled:opacity-50"
            >
              Search
            </button>
          </div>
...
const SectionHeader = ({ total, searchTerm, activeTag }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center mb-8 border-b border-white/5 pb-4">
      <span className="text-[10px] tracking-[0.4em] text-[#555] font-mono uppercase text-center">
        {title}
      </span>
      {total > 0 && (
        <span className="text-white text-[9px] tracking-[0.2em] font-mono mt-2 uppercase">
...
const EmptyState = ({ isFiltering }) => (
  <div className="flex flex-col items-center justify-center py-32 opacity-40 border border-dashed border-white/5 mx-4">
    <span className="text-3xl mb-4 font-serif text-white italic text-center">
      {isFiltering ? "No Matching Records" : "The Board is Empty"}
    </span>
    <p className="text-[10px] font-mono tracking-widest uppercase text-[#888] text-center">
      {isFiltering
...
const HomePage = () => {
  ...
  // Main fetcher: now accepts page, search, AND tag
  ...
  // Trigger fetch when search or tag changes
  useEffect(() => {
  ...
  const toggleTag = (tag) => {
    // If clicking the already active tag, turn it off. Otherwise, set it.
  ...
  const handleDeleted = (id) => {
    ...
    if (confessions.length === 1 && pagination.page > 1) {
      loadConfessions(pagination.page - 1, activeSearch, activeTag);
    }
  };
  ...
  return (
    <Layout onConfessionCreated={handleCreated}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative">
        ...
        <ArchiveSearch onSearch={handleSearch} isSearching={loading} />
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mb-10 anim-slide-up delay-0">
            {predefinedTags.map((tag) => (
              <button
                ...
                className={`text-[9px] sm:text-[10px] uppercase tracking-[0.15em] border px-3 py-1.5 transition-all duration-300 ${
                  activeTag === tag
                    ? "bg-white text-black border-white font-bold scale-105 shadow-[0_0_15px_rgba(255,255,255,0.2)]"
                    : "bg-transparent text-[#666] border-white/10 hover:border-white/40 hover:text-white"
                }`}
              >
                #{tag}
              </button>
